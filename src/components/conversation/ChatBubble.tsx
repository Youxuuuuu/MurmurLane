...
function ChatBubbleContent({
  ...
  const bubbleMotionKey = (part: string, index: number) =>
    `${bubbleKeyRoot}:part:${index}:${part}`;
  const bubbleMotion = (_part: string, index: number) =>
    isAnimatingBubbleSequence
      ? {
          initial: { opacity: 0, transform: "translateY(8px) scale(0.975)" },
      ...
      >
        <div className={`max-w-[280px] ${fromUser ? "text-right" : "text-left"}`}>
          <div className={`flex flex-col gap-2 ${fromUser ? "items-end" : "items-start"}`}>
            {(displayText ? splitBubbleText(displayText) : []).map((part, index) => (
              <motion.div
                key={bubbleMotionKey("__quote_text__", index)}
                data-bubble-text={part}
                className={`${fromUser ? "border-black/[0.06] bg-[#f3f3f2] text-black/[0.78]" : "bg-white/[0.73] text-black/[0.72]"} inline-block rounded-[7px] border px-3 py-1.5 text-left font-sans text-[14px] font-normal leading-[1.55]`}
                style={{
                  borderColor: fromUser ? "rgba(0,0,0,.06)" : page.line,
                  transformOrigin: fromUser ? "right bottom" : "left bottom",
                  willChange: isAnimatingBubbleSequence ? "transform, opacity" : undefined,
      ...
      >
        <div className={`flex max-w-[min(78vw,360px)] flex-col gap-2 ${fromUser ? "items-end" : "items-start"}`}>
          ...
          <motion.div
            ...
            {...bubbleMotion("__voice__", messageTextParts.length)}
          >
      ...
      >
        <div className={`flex max-w-[min(78vw,360px)] flex-col gap-2 ${fromUser ? "items-end" : "items-start"}`}>
          ...
          <motion.div
            ...
            {...bubbleMotion("__file__", messageTextParts.length)}
          >
            <div
        ...
        >
          <div className={`flex max-w-[min(92vw,360px)] flex-col gap-2 ${fromUser ? "items-end" : "items-start"}`}>
            ...
            <motion.div
              ...
              {...bubbleMotion("__image__", messageTextParts.length)}
            >
              <div
      ...
      >
        <div className={`flex max-w-[min(78vw,360px)] flex-col gap-2 ${fromUser ? "items-end" : "items-start"}`}>
          ...
          <motion.div
            ...
            {...bubbleMotion("__media__", messageTextParts.length)}
          >
